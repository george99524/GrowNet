...
from data.data import LibCSVData, LibSVMData, LibSVMRegData, LibTXTData
...
from models.mlp import MLP_1HL, MLP_2HL, MLP_3HL, BayesianRegressor
...
def get_data():
    ...
    elif opt.data in ["sunspot"]:
        train = LibTXTData(opt.tr, opt.feat_d, opt.normalization)
        test = LibTXTData(opt.te, opt.feat_d, opt.normalization)
        val = []
        if opt.cv:
            val = copy.deepcopy(train)
            # print("Creating Validation set! \n")
...
def run_experiment():
    ...
    for stage in range(opt.num_nets):
        t0 = time.time()
        # model = MLP_2HL.get_model(
        #     stage, opt
        # )  # Initialize the model_k: f_k(x), multilayer perception v2
        model = model_type.get_model(stage, opt)
        if opt.cuda:
            model.cuda()
        ...
        net_ensemble = DynamicNet.from_file(
            opt.out_f, lambda stage: model_type.get_model(stage, opt)
        )
...
if __name__ == "__main__":
    ...
    for level in [2, 3, 4, 5]:
        ...
        te_rmse_l = []
        total_time = 0
        for _ in range(opt.exps):
            tr_rmse, te_rmse, runtime = run_experiment()
        ...
        print(
            f"Training statistics: mean - {np.mean(tr_rmse_l)}, best = {np.amin(tr_rmse_l)}, std = {np.std(tr_rmse_l)}"
        )
        print(
            f"Test statistics: mean - {np.mean(te_rmse_l)}, best = {np.amin(te_rmse_l)}, std = {np.std(te_rmse_l)}"
        )
